...
router.post('/getMyselfInformation',function (req,res,next) {
    ...
    form.parse(req,function (err,fields,file) {
        ...
        let promise=new Promise(function (resolve,reject) {
          userModels
              .findOneUserById(req.session.user.userId)
              .then(function (result) {
                  delete result.password;
                  delete result._id;

                  myself=result;
                  myself.dormitoryMemberName=[];
                  resolve({});
              })
        }).then(function (data) {
            if (req.session.user.dormitoryId){
                    ...
                    new Promise(function (resolve,reject) {
                            dormitoryModels
                                ...
                                .then(function (result) {
                                    myself.dormitoryMemberId=result.dormitoryMenber.split(',');
                                    resolve({});
                                })
                        })
                        .then(function (data) {
                        return new Promise(function (resolve, reject) {
                            powerModels
                                ...
                                .then(function (result) {
                                    result.delete ? myself.dormitoryIdentity = '宿舍长' : myself.dormitoryIdentity = '宿舍成员';
                    ...
                    .then(function (data) {
                        for (let i of myself.dormitoryMemberId){
                            promise.then(function (data) {
                                return new Promise(function (resolve, reject) {
                                    let dormitoryMemberId=i;
                                    userModels
                                        .findOneUserById(dormitoryMemberId)
                                        .then(function (result) {
                                            myself.dormitoryMemberName.push(result.name);
                                            if (i==myself.dormitoryMemberId[myself.dormitoryMemberId.length-1]){
    ...
    form.parse(req,function (err,fields,file) {
        ...
        userModels.findOneUsername(fields.aimName[0])
            .then(function (result) {
                ...
                dormitoryModels.dormitoryInformationById(req.session.user.dormitoryId)
                    .then(function (result) {
                        try {
                            if (!result){
                                throw new Error('请先创建你的宿舍');
                            }
                            if (!result.dormitoryMenber.split(',').indexOf(aimUserId)==-1){
                        ...
                        powerModels.getPowerByuserId(req.session.user.userId)
                            .then(function (result) {
                                ...
                                powerModels
                                    .transferOfAuthority(req.session.user.userId,aimUserId.toString())
                                    .then(function (result) {
                                        res.json({
                                            'status':'200',
                                            'msg':'成功转让权限'
            ...
            .then(function (result) {
                ...
                userModels.findOneUsername(fields.aimName[0])
                    .then(function (result) {
                        ...
                        let messageIdQueue='';
                        result.messageIdQueue?messageIdQueue=result.messageIdQueue+',': result.messageIdQueue='';
                        powerModels
                            .getPowerByuserId(req.session.user.userId)
                            .then(function (result) {
